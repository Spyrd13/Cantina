...
class ClientesView(ft.Column):
                ...
                cells=[
                    ...
                    ft.DataCell(
                        ft.Text(
                            ...
                            color=ft.Colors.RED_600 if (c.saldo_devedor or 0) > 0 else ft.Colors.GREEN_600,
                            weight=ft.FontWeight.BOLD if (c.saldo_devedor or 0) > 0 else ft.FontWeight.NORMAL,
                        )
                    ),
                    ft.DataCell(
                        ft.Row([
    ...
    def _on_busca(self, e):
        ...
        if termo:
            clientes = self.service.buscar_por_nome(termo)
        else:
            clientes = self.service.listar_todos()
    ...
    def _filtrar_devedores(self, e):
        if self._mostrando_devedores:
            ...
            self.field_busca.value = ""
            clientes = self.service.listar_devedores()
            self._carregar_tabela(clientes)
    ...
    def _salvar(self, e):
        ...
        try:
            if self._cliente_editando_id is None:
                self.service.cadastrar(ClienteCreate(
                    nome=nome,
